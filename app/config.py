...
class Settings(BaseSettings):
    ...
    # Текстовая модель для режима "улучшить шаг" (грамматика/полировка текста).
    # Это чисто текстовая задача — тяжёлую vision-модель сюда тащить незачем.
    # Маленькая qwen2.5:3b (~2.4 ГБ) легко влезает на GPU и работает быстро.
    TEXT_MODEL: str = Field(
        default="qwen2.5:3b",
        description="Название текстовой LLM для улучшения текста шагов (без vision)"
    )
