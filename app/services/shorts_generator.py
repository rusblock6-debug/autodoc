...
class ShortsGenerator:
    ...
    async def generate_from_steps(
        ...
        Args:
            ...
            tts_engine: Движок TTS - "edge" (быстро) или "chatterbox" (медленно, качественнее)
            
        Returns:
            ShortsResult с путём к видео
        """
        logger.info(f"Generating Shorts for guide {guide_uuid} with {len(steps)} steps using {tts_engine} TTS")
        ...
        try:
            ...
            for i, step in enumerate(steps):
                ...
                # Генерируем TTS (асинхронно для обоих сервисов)
    ...
    async def _create_segment_video(
        ...
        cmd = [
            "ffmpeg",
            "-nostdin",  # Не ждать ввода от пользователя
            "-y",
            "-loop", "1",
            "-t", str(segment.duration_seconds),  # Длительность ПЕРЕД входным файлом
        ...
        try:
            ...
            # Запускаем FFmpeg через asyncio subprocess
            logger.info(f"Starting FFmpeg process via asyncio...")
            ...
            if process.returncode == 0 and output_path.exists():
                logger.info(f"Segment video created successfully: {output_path}")
