    ...
    def analyze_screenshot(
        ...
        try:
            ...
            if has_click:
                ...
                # Глагольный блок зависит от типа действия, записанного
                # расширением. Для select/drag/input глагол НЕ угадывается
                # по картинке — он известен достоверно из записи.
                if action == "select":
                    verb_block = (
                        "Пользователь ВЫДЕЛИЛ мышью текст — маркер стоит на месте выделения.\n"
                        "Инструкция ОБЯЗАНА начинаться с «Выделите …»: назови, какой текст "
                        "или фрагмент выделить (возьми из подсказки или с картинки)."
                    )
                    examples_block = (
                        '✅ "Выделите название статьи в списке"\n'
                        '✅ "Выделите адрес ссылки в адресной строке"'
                    )
                elif action == "drag":
                    verb_block = (
                        "Пользователь ПЕРЕТАЩИЛ элемент (drag-and-drop) — маркер стоит на точке захвата.\n"
                        "Инструкция ОБЯЗАНА начинаться с «Перетащите …»: назови, ЧТО перетащить, "
                        "и, если понятно по картинке, куда."
                    )
                    examples_block = (
                        '✅ "Перетащите карточку задачи в колонку «Готово»"\n'
                        '✅ "Перетащите ползунок громкости вправо"'
                    )
                elif action == "input":
                    verb_block = (
                        "Пользователь ВВЁЛ значение в поле — маркер стоит на поле ввода.\n"
                        "Инструкция ОБЯЗАНА начинаться с «Введите …» (для выпадающего списка — «Выберите …»): "
                        "назови поле и, если есть в подсказке, само значение."
                    )
                    examples_block = (
                        '✅ "Введите адрес электронной почты в поле «Email»"\n'
                        '✅ "Выберите «Россия» в списке «Страна»"'
                    )
                else:
                    verb_block = """ШАГ 1. Мысленно определи ТИП элемента под маркером:
